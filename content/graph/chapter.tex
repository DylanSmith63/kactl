...
\section{Network flow}
	\kactlimport{PushRelabel.h} % keep (used by GomoryHu)
	\kactlimport{MinCut.h} % keep
	\kactlimport{MinCostMaxFlow.h} % keep
	\kactlimport{GlobalMinCut.h} % keep
	\kactlimport{GomoryHu.h} % keep
	\kactlimport{FlowWithDemands.h} % keep
	\kactlimport{Dinic.h} % keep
	% \kactlimport{EdmondsKarp.h} % remove
...
\subsection{Flow Notes}
	...
	Hall's Theorem: In a bipartite graph $G = (L, R, E)$, there exists a matching that covers $L$ iff for every subset $S \subseteq L$, $|N(S)| \geq |S|$ where $N(S)$ is the set of neighbors of $S$

\section{Matching}
	\kactlimport{hopcroftKarp.h} % keep
	\kactlimport{DFSMatching.h} % keep (short)
	\kactlimport{MinimumVertexCover.h} % keep (probably)
	\kactlimport{MaximumIndependentSet.h} % keep
	\kactlimport{WeightedMatching.h} % keep
	\kactlimport{GeneralMatching.h} % keep

\section{Matroid}
	\kactlimport{MatroidIntersection.h} % keep
    ...
    \begin{itemize}
     ...
     \item Find ``free to include'' sets $Y_1$ and $Y_2$
     \item Find augmenting path $P$ \textbf{without shortcuts (BFS)} from an element in $Y_1$ to an element in $Y_2$
     \item Alternate inclusion into $S$ of all element in $P$
    \end{itemize}
...
\section{DFS algorithms}
	\kactlimport{SCC.h} % keep
	\kactlimport{BiconnectedComponents.h} % keep
	\kactlimport{Articulation.h} % keep
	% \kactlimport{2sat.h} % remove because we can rewrite from memory
	\kactlimport{EulerWalk.h} % keep

\section{Coloring}
	\kactlimport{EdgeColoring.h} % keep but note that its for non-bipartite graphs

\section{Heuristics}
	\kactlimport{MaximalCliques.h} % keep
	\kactlimport{MaximumClique.h} % keep

\section{Trees}
	\kactlimport{CompressTree.h} % keep
	% \kactlimport{HLD.h} % remove
	\kactlimport{LinkCutTree.h} % keep but im not comfotable using it
	\kactlimport{DirectedMST.h} % keep
	\kactlimport{LCA.h} % keep
